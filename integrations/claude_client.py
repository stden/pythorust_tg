"""
Claude API client for integration with Anthropic Claude.
...
class ClaudeClient:
    """
    Client for working with the Anthropic Claude API.
    ...
    model: str = "claude-sonnet-4-5-20250929"  # Latest Claude model.
    max_tokens: int = 4096
    ...
    async def chat_stream(
        ...
        """
        Streaming response from Claude (Server-Sent Events).
...
async def quick_chat(message: str, model: str = "claude-4-sonnet") -> str:
    """
    Quick Claude chat without creating a client.

    Args:
        message: User message
        model: Model (defaults to claude-4-sonnet)
