...
impl LinearClient {
    ...
    pub fn new<S: Into<String>>(api_key: S) -> Result<Self> {
        ...
        Ok(Self {
            http,
            api_key,
            base_url: std::env::var("LINEAR_API_URL").unwrap_or_else(|_| LINEAR_API_URL.to_string()),
            team_cache: Arc::new(Mutex::new(HashMap::new())),
        })
    }

    /// Create client with custom base url (primarily for tests).
    pub fn with_base_url<S1: Into<String>, S2: Into<String>>(api_key: S1, base_url: S2) -> Result<Self> {
        let mut client = Self::new(api_key)?;
        client.base_url = base_url.into();
        Ok(client)
    }

    async fn post<V: Serialize, D: for<'de> Deserialize<'de>>(&self, query: &'static str, variables: V) -> Result<D> {
        let response = self
            .http
            .post(&self.base_url)
        ...
        let envelope: GraphQlResponse<D> = serde_json::from_str(&text)
            .map_err(|e| Error::LinearError(format!("Linear вернул не-JSON ответ: {} ({})", text, e)))?;
    ...
    pub async fn get_team_id(&self, team_key: &str) -> Result<String> {
        if let Some(cached) = self
            .team_cache
            .lock()
            .map_err(|_| Error::LinearError("Не удалось получить доступ к кешу команд".to_string()))?
            .get(team_key)
            .cloned()
        {
            return Ok(cached);
        ...
        let data: TeamResponse = self.post(TEAM_QUERY, TeamVariables { key: team_key }).await?;
        let team = data
            .team
            .ok_or_else(|| Error::LinearError(format!("Команда '{}' не найдена в Linear", team_key)))?;
    ...
    pub async fn create_issue(&self, input: CreateIssueInput) -> Result<CreatedIssue> {
        if input.title.trim().is_empty() {
            return Err(Error::InvalidArgument("Заголовок задачи пустой".to_string()));
        }
        ...
        let issue = data
            ...
            .ok_or_else(|| Error::LinearError("Linear не подтвердил создание задачи".to_string()))?;
...
mod tests {
    ...
    async fn create_issue_success() {
        ...
        let client = setup_client(&server);
        let issue = client.create_issue(CreateIssueInput {
            team_key: "APP".into(),
            title: "Title".into(),
        ...
        assert_eq!(issue.url.as_deref(), Some("https://linear.app/app-1/issue/APP-101/title"));
        team_mock.assert_calls(1);
    ...
    async fn create_issue_propagates_linear_error() {
        ...
        server.mock(|when, then| {
            ...
            then.status(200)
                .json_body(serde_json::json!({"data": { "team": { "id": "team-123", "key": "APP" } }}));
        });
        ...
        let client = setup_client(&server);
        let err = client.create_issue(CreateIssueInput {
            team_key: "APP".into(),
            title: "Broken".into(),
            description: None,
            project_id: None,
    ...
    fn from_optional_key_uses_env_when_missing_argument() {
        ...
        let _url_guard = EnvGuard::clear("LINEAR_API_URL");
