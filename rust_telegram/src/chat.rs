...
pub async fn resolve_chat(client: &Client, entity: &ChatEntity) -> Result<Peer> {
    match entity {
        ChatEntity::Channel(target_id) => {
            ...
            while let Some(dialog) = dialogs.next().await.map_err(|e| Error::TelegramError(e.to_string()))? {
            ...
            Err(Error::ChatNotFound(format!("Channel {} not found in dialogs", target_id)))
        }
        ChatEntity::Chat(target_id) => {
            ...
            Err(Error::ChatNotFound(format!("Chat {} not found in dialogs", target_id)))
        }
        ...
        ChatEntity::UserId(target_id) => {
            ...
            Err(Error::ChatNotFound(format!("User {} not found in dialogs", target_id)))
        }
    }
}
...
pub fn peer_name(peer: &Peer) -> String {
    peer.name().map(|s| s.to_string()).unwrap_or_else(|| "Unknown".to_string())
}
