...
pub async fn index_chat(
    ...
    let mut count = 0;
    let mut user_stats: std::collections::HashMap<i64, UserStats> = std::collections::HashMap::new();
    ...
    while let Some(message) = messages_iter.next().await? {
        ...
        if let Some(analyzed) = analyze_message(&message, chat_name, chat_id).await {
            // Update user stats
            let stats = user_stats.entry(analyzed.sender_id).or_insert_with(|| UserStats {
    ...
    info!("Collected {} messages from {}", analyzed_messages.len(), chat_name);
...
async fn analyze_message(message: &Message, chat_name: &str, chat_id: i64) -> Option<AnalyzedMessage> {
    ...
    let mut analyzed = AnalyzedMessage::new(
        ...
        sender_name,
        text.to_string(),
        DateTime::from_timestamp(message.date().timestamp(), 0)
...
fn estimate_sentiment(text: &str) -> f32 {
    ...
    let positive = ["хорошо", "отлично", "супер", "круто", "спасибо", "люблю", "рад", "счаст", "❤️", "👍", "🔥", "😊", "😍"];
    let negative = ["плохо", "ужас", "грустно", "жаль", "злой", "ненавиж", "проблем", "😢", "😭", "👎", "😠"];
