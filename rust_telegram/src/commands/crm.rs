...
//!
//! Based on the CRM idea from vibecod3rs chat - automatically parse conversations
...
pub async fn parse_chat(chat_name: &str, config: CrmConfig) -> Result<CrmExtraction> {
        ...
        if let Ok(msg) = msg_result {
            ...
            let sender = if let Some(sender) = msg.sender() {
                match sender {
                    ...
                    grammers_client::types::Peer::Group(g) => g.title().unwrap_or("Group").to_string(),
                }
            } else {
                "Unknown".to_string()
            };
    ...
    let conversation = messages
        .iter()
        .map(|(sender, text, ts)| {
            format!("[{}] {}: {}", ts.format("%d.%m %H:%M"), sender, text)
        })
        .collect::<Vec<_>>()
        .join("\n");
...
async fn extract_crm_data(
        ...
        messages: vec![
            ...
            ChatCompletionRequestMessage::User(ChatCompletionRequestUserMessage {
                content: async_openai::types::ChatCompletionRequestUserMessageContent::Text(
                    format!("Проанализируй эту переписку:\n\n{}", conversation),
                ),
                name: None,
            }),
        ],
    ...
    // Parse JSON response
    let extraction: CrmExtraction = serde_json::from_str(&content)
        .unwrap_or_else(|e| {
