...
async fn main() -> Result<()> {
    ...
    println!("  Успешных:        {} ({:.1}%)", success,
        if total_requests > 0 { success as f64 / total_requests as f64 * 100.0 } else { 0.0 });
    println!("  Ошибок:          {} ({:.1}%)", errors,
        if total_requests > 0 { errors as f64 / total_requests as f64 * 100.0 } else { 0.0 });
    println!("───────────────────────────────────────────");
    ...
    println!("    2xx:           {}", stats.status_2xx.load(Ordering::Relaxed));
    println!("    3xx:           {}", stats.status_3xx.load(Ordering::Relaxed));
    println!("    4xx:           {}", stats.status_4xx.load(Ordering::Relaxed));
    println!("    5xx:           {}", stats.status_5xx.load(Ordering::Relaxed));
    println!("═══════════════════════════════════════════");
