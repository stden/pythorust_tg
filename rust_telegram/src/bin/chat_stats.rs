...
async fn main() -> Result<()> {
    ...
    if let (Some(first), Some(last)) = (first_msg_date, last_msg_date) {
        ...
        println!("Date range: {} to {}", first.format("%Y-%m-%d"), last.format("%Y-%m-%d"));
        println!("Messages per day: {:.1}", total_messages as f64 / days as f64);
    }
    ...
    println!("\n=== Top {} Senders ===\n", cli.top);
    println!("{:<4} {:<25} {:>8} {:>10} {:>8}", "#", "Name", "Messages", "Chars", "Avg len");
    println!("{}", "-".repeat(60));
