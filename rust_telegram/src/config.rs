...
impl Config {
    ...
    pub fn load_from_file<P: AsRef<Path>>(path: P) -> Result<Self, String> {
        ...
        Ok(Self {
            phone: telegram.phone.unwrap_or_default(),
            api_id: telegram.api_id.unwrap_or(0),
            api_hash: telegram.api_hash.unwrap_or_default(),
            session_name: telegram.session_name.unwrap_or_else(|| SESSION_NAME.to_string()),
            lock_file: LOCK_FILE.to_string(),
            my_user_id: user.id.unwrap_or(0),
