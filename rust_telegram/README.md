...
- Инструмент: `cargo-mutants` (конфиг `.cargo/mutants.toml` — пропускаем `src/bin/**`, увеличены таймауты, отчеты в `target/mutants`).
- Установка: `cargo install --locked cargo-mutants`
- Запуск из каталога `rust_telegram`: `cargo mutants --no-shuffle --jobs 4`
- Посмотреть список мутантов без прогона тестов: `cargo mutants --list`
