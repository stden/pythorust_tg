...
# Import Rust implementations for hot path functions
try:
    from telegram_reader import detect_conversion as _rust_detect_conversion
...
class ABTestManager:
    ...
    def detect_conversion(self, text: str) -> Optional[str]:
        ...
        if _USE_RUST:
            return _rust_detect_conversion(text or "")
        return self._detect_conversion_python(text)

    def _detect_conversion_python(self, text: str) -> Optional[str]:
        """Python fallback for detect_conversion."""
        if not text:
            return None
