...
- Schema source of truth: `migrations/002_create_bot_tables.sql`. Apply it
  before running `community_game_bot` or `credit_expert_bot`; `sales_bot`
  creates the same tables on startup via `ensure_tables()`.
