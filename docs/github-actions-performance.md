...
| Use the mold linker | This crate links 53 Cargo binary targets (main CLI + 52 specialized binaries) plus a cdylib against heavy deps; linking, not codegen, is the bottleneck. mold links 2-5x faster than GNU `ld`. | `rui314/setup-mold` + `RUSTFLAGS=-Clink-arg=-fuse-ld=mold` on the debug Rust jobs. |
| Lockfile policy split | Python keeps local-only `uv.lock`; Rust commits `Cargo.lock` for reproducible application builds. | CI currently passes no `--locked`/`--frozen`. `Cargo.lock` is watched by Rust path filters; the yanked `core2` chain is handled by the temporary `vendor/core2-0.4.0` patch tracked in `AGENTS.md`. |
...
### 4. Keep Cargo.lock Committed

This crate ships many binaries, so Rust builds should be pinned with a committed
`Cargo.lock`. The workflow path filters include `Cargo.lock`, and the current
tree includes both the lockfile and a temporary `[patch.crates-io]` workaround
for the yanked transitive dependency chain (`grammers-crypto` →
`glass_pumpkin` → `core2 0.4.0`).

CI currently does **not** pass `--locked`; cargo may still resolve from
`Cargo.toml` each run:
...
The next CI hardening step is to verify the full Rust gate with the committed
lockfile, then add `--locked` to these commands. That prevents CI from silently
resolving dependency versions differently from the checked-in lockfile.
...
links **53 Cargo binary targets (main CLI + 52 specialized bins) + a PyO3
cdylib** against tokio, pyo3, teloxide, kube, grammers, reqwest, and tonic. The
link phase dominates the debug jobs, and `mold` is a drop-in replacement for GNU
`ld` that links 2-5x faster.
...
- Add `--locked` only after a full CI-equivalent Rust gate is green with the
