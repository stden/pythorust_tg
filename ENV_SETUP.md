...
- `TELEGRAM_SESSION_NAME` / `TELEGRAM_SESSION_FILE` name the Telegram session; leave them as `telegram_session` unless you need a different session name (also used by the remaining Python tools, e.g. `mcp_telegram_server.py`).
