...
pub static KNOWN_SENDERS: LazyLock<HashMap<i64, &'static str>> = LazyLock::new(|| {
    // Empty by default; populated from config.yml.
    HashMap::new()
});
...
impl Config {
    ...
    pub fn load_from_file<P: AsRef<Path>>(path: P) -> Result<Self, String> {
        ...
        Ok(Self {
            ...
            session_name: {
                let value =
                    Self::resolve_env_string(telegram.session_name, "TELEGRAM_SESSION_NAME");
                if value.is_empty() {
            ...
            lock_file: env_or_default("TELEGRAM_LOCK_FILE", LOCK_FILE),
            my_user_id,
    ...
    fn defaults() -> Self {
        Self {
            ...
            api_id: 0,
            api_hash: String::new(),
            session_name: env_or_default("TELEGRAM_SESSION_NAME", SESSION_NAME),
            lock_file: env_or_default("TELEGRAM_LOCK_FILE", LOCK_FILE),
            my_user_id: 0,
...
"#;
    ...
    #[test]
    fn config_defaults_reads_session_paths_from_env() {
        let _lock = ENV_LOCK.lock().unwrap();
        let _guards = set_envs(&[
            ("TELEGRAM_SESSION_NAME", "custom_session"),
            ("TELEGRAM_LOCK_FILE", "custom.lock"),
        ]);
...
"#;
        ...
        match config.chats.get("my_group") {
            Some(ChatEntity::Chat(id)) => assert_eq!(*id, -1001),
            _ => panic!("Expected Chat variant"),
        }
