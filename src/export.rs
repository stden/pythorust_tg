...
mod tests {
    ...
    fn writes_header_and_messages() -> crate::error::Result<()> {
        ...
        writer.write_message("EntityB", "No media", "", None, None)?;
        writer.finish()?;

        let contents = std::fs::read_to_string("chat_test.md")?;
        assert!(contents.starts_with("Prompt line\n"));
        assert!(contents.contains("22.11.2024 10:30:05 EntityA: Hello 🔥 media/photo.jpg"));
        assert!(contents.contains("EntityB: No media"));
