...
mod tests {
    ...
    async fn retrieves_relevant_chunks_locally() {
        ...
        rag.ingest(
            "doc1",
            "EntityA loves Rust programming and open source projects.",
        )
        .await
        .unwrap();
        rag.ingest(
            "doc2",
            "Gardening on weekends helps EntityB relax and enjoy nature.",
        )
        .await
        .unwrap();

        let results = rag
            .retrieve("What does EntityA love to code?", 3, RetrievalMode::Hybrid)
            .await
            .unwrap();
    ...
    async fn retrieve_with_graph_only_mode() {
        ...
        rag.ingest("doc1", "EntityA collaborates with EntityB on Rust projects")
            .await
            .unwrap();

        let results = rag
            .retrieve("EntityA", 2, RetrievalMode::GraphOnly)
            .await
            .unwrap();
