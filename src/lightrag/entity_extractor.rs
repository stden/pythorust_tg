...
mod tests {
    ...
    fn extracts_entities_and_relations() {
        ...
        let chunk = Chunk::new(
            "EntityA met EntityB in CityA with @handle_c".to_string(),
            0,
            6,
            "test",
        );
        ...
        assert!(entities.iter().any(|e| e.name.starts_with("EntityA")));
        assert!(entities.iter().any(|e| e.name.starts_with("EntityB")));
        assert!(entities.iter().any(|e| e.name.contains("@handle_c")));
    ...
    fn extracts_mentions() {
        let extractor = EntityExtractor::new();
        let chunk = Chunk::new("Thanks @handle_a and @handle_b".to_string(), 0, 4, "test");
        let (entities, _) = extractor.extract(&chunk);

        assert!(entities.iter().any(|e| e.name.contains("@handle_a")));
        assert!(entities.iter().any(|e| e.name.contains("@handle_b")));
    }
    ...
    fn creates_co_occurrence_relations() {
        ...
        assert_eq!(relations.len(), 2); // EntityA-EntityB, EntityB-EntityC
    ...
    fn deduplicates_entities() {
        let extractor = EntityExtractor::new();
        let chunk = Chunk::new(
            "EntityA met EntityA again EntityA".to_string(),
            0,
            5,
            "test",
        );
        ...
        // Should only have one EntityA
        let alice_count = entities.iter().filter(|e| e.name == "EntityA").count();
    ...
    fn entity_stores_chunk_id() {
        let extractor = EntityExtractor::new();
        let chunk = Chunk::new("EntityA".to_string(), 0, 1, "test");
        let (entities, _) = extractor.extract(&chunk);
    ...
    fn extractor_default() {
        ...
        let chunk = Chunk::new("EntityA met EntityB".to_string(), 0, 3, "test");
        let (entities, _) = extractor.extract(&chunk);
