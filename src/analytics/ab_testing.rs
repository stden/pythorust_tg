...
mod tests {
    ...
    fn test_detect_conversion_phone() {
        ...
        assert_eq!(
            tm.detect_conversion("+0 000 000 00 00"),
            Some("phone_shared")
        );
        assert_eq!(tm.detect_conversion("00000000000"), Some("phone_shared"));
    ...
    fn test_detect_conversion_purchase_intent() {
        ...
        impl TestManager {
            fn detect_conversion(&self, text: &str) -> Option<&'static str> {
                ...
                let normalized = text.to_lowercase();
                let intent_keywords = ["беру", "покупаю", "оформляем", "оплачиваю", "готов купить"];
        ...
        assert_eq!(
            tm.detect_conversion("Готов купить!"),
            Some("purchase_intent")
        );
    ...
    fn test_prompt_variant_clone() {
        let variant = PromptVariant::new("original", "Original prompt").with_weight(2.0);
