...
impl SessionLock {
    ...
    pub fn acquire() -> Result<Self> {
        let lock_path = Config::new().lock_file;
...
"#
    ...
    pub fn release(&mut self) {
        ...
        self.lock_file = None;
        let _ = std::fs::remove_file(Config::new().lock_file);
    }
}
...
fn session_file_path() -> String {
    let value =
        std::env::var("TELEGRAM_SESSION_FILE").unwrap_or_else(|_| Config::new().session_name);
    let stem = value.strip_suffix(".session").unwrap_or(&value);
    format!("{}.session", stem)
}
...
mod tests {
    use super::*;
    use crate::config::{LOCK_FILE, SESSION_NAME};
    use std::env;
