...
static PROCESS_COLLECTOR: Lazy<()> = Lazy::new(|| {
    info!("Process collector is not supported on this platform, skipping registration.");
});
...
mod tests {
    ...
    async fn metrics_response_contains_inflight_gauge() {
        // IntGaugeVec exports no time series until a labeled child exists, so a
        // command must be recorded as in-flight before the gauge appears. Without
        // this the test only passed when another test in the same process happened
        // to touch the gauge first (order-dependent flake).
        let cmd = "test_inflight_check";
        record_command_start(cmd);
