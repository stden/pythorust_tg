...
//! - Action requests (@stden do, need, must)
...
const TASK_KEYWORDS: &[&str] = &[
    "сделай", "нужно", "надо", "можешь", "помоги", "исправь", "добавь",
    "удали", "настрой", "проверь", "запусти",
];

const QUESTION_KEYWORDS: &[&str] = &["?", "как", "почему", "что делать", "где", "когда", "кто"];

const PROBLEM_KEYWORDS: &[&str] = &["не работает", "ошибка", "баг", "сломалось", "проблема", "упало", "крашится", "зависло"];
...
impl ChatTaskChecker {
    ...
    async fn check_chat(&mut self, _client: &grammers_client::Client, message: &Message, chat_name: &str) -> Result<()> {
        ...
        if text_lower.contains(&format!("@{}", self.my_username.to_lowercase())) 
           || (TASK_KEYWORDS.iter().any(|k| text_lower.contains(k)) && text.len() > 20) {
            self.tasks_by_chat.entry(chat_name.to_string()).or_default().push(entry.clone());
        }

        // Check for questions
        if QUESTION_KEYWORDS.iter().any(|k| text_lower.contains(k)) {
            self.questions_by_chat.entry(chat_name.to_string()).or_default().push(entry.clone());
        }

        // Check for problems
        if PROBLEM_KEYWORDS.iter().any(|k| text_lower.contains(k)) {
            self.problems_by_chat.entry(chat_name.to_string()).or_default().push(entry);
        }
    ...
    fn generate_report(&self, output_file: &PathBuf) -> Result<()> {
        ...
        lines.push(format!("**Дата проверки:** {}", Utc::now().format("%Y-%m-%d %H:%M:%S")));
        lines.push("".to_string());

        let all_chats: std::collections::HashSet<_> = self.tasks_by_chat.keys()
        ...
        if !self.tasks_by_chat.is_empty() {
            ...
            sorted_chats.sort_by(|a, b| b.1.len().cmp(&a.1.len()));
            ...
            for (chat_name, tasks) in sorted_chats {
                ...
                sorted_tasks.sort_by(|a, b| b.date.cmp(&a.date));

                for task in sorted_tasks.iter().take(5) {
                    lines.push(format!("**[{}] {}:**", task.date.format("%Y-%m-%d %H:%M"), task.sender));
        ...
        if !self.questions_by_chat.is_empty() {
            ...
            for (chat_name, questions) in sorted_chats {
                if questions.len() < 3 { continue; }
                lines.push(format!("### {} ({} вопросов)", chat_name, questions.len()));
                lines.push("".to_string());

                let mut sorted_q = questions.clone();
                sorted_q.sort_by(|a, b| b.date.cmp(&a.date));

                for q in sorted_q.iter().take(3) {
                    lines.push(format!("**[{}] {}:**", q.date.format("%Y-%m-%d %H:%M"), q.sender));
        ...
        if !self.problems_by_chat.is_empty() {
            ...
            for (chat_name, problems) in sorted_chats {
                ...
                sorted_p.sort_by(|a, b| b.date.cmp(&a.date));

                for p in sorted_p.iter().take(5) {
                    lines.push(format!("**[{}] {}:**", p.date.format("%Y-%m-%d %H:%M"), p.sender));
...
async fn main() -> Result<()> {
    ...
    let args = Args::parse();
    let my_username = env::var("MY_NAME").unwrap_or_else(|_| "stden".to_string());
