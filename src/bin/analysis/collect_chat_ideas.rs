...
//! - Improvement ideas
//!
//! Usage: cargo run --bin collect_chat_ideas -- --chats @example_channel [--days 7] [--limit 500] [--output analysis_results/chat_ideas.md]
...
#[derive(Parser)]
struct Args {
    /// Chats to analyze (e.g. @example_channel)
...
impl IdeaCollector {
    ...
    async fn collect_from_chat(
        ...
        println!(
            "  💡 Найдено идей: {}",
            self.ideas.get(&chat_name).map_or(0, |v| v.len())
        );
        println!(
            "  ⚠️  Найдено проблем: {}",
            self.problems.get(&chat_name).map_or(0, |v| v.len())
        );
        println!(
            "  🔧 Упомянуто инструментов: {}",
            self.tools.get(&chat_name).map_or(0, |v| v.len())
        );
    ...
    fn generate_report(&self, output_file: &PathBuf) -> Result<()> {
        ...
        for chat_name in chat_names {
            if let Some(ideas) = self.ideas.get(chat_name) {
                ...
                sorted_ideas.sort_by_key(|x| std::cmp::Reverse(x.date));

                for idea in sorted_ideas.iter().take(10) {
                    lines.push(format!(
                        "**[{}] {}:**",
                        idea.date.format("%Y-%m-%d %H:%M"),
                        idea.sender
                    ));
        ...
        for chat_name in prob_chat_names {
            if let Some(problems) = self.problems.get(chat_name) {
                ...
                sorted_problems.sort_by_key(|x| std::cmp::Reverse(x.date));

                for problem in sorted_problems.iter().take(10) {
                    lines.push(format!(
                        "**[{}] {}:**",
                        problem.date.format("%Y-%m-%d %H:%M"),
                        problem.sender
                    ));
        ...
        sorted_counts.sort_by_key(|x| std::cmp::Reverse(x.1));
        ...
        lines.push(
            "3. **Интеграции:** Рассмотреть интеграции с популярными инструментами".to_string(),
        );
...
async fn main() -> Result<()> {
    ...
    for chat in &args.chats {
        collector
            .collect_from_chat(&client, chat, args.days, args.limit)
            .await?;
    }
