...
struct Args {
    ...
    #[arg(short = 'c', long = "command", value_delimiter = ',', default_value = "/start,/status")]
    commands: Vec<String>,
...
async fn latest_message_id(client: &telegram_reader::session::TelegramClient, peer: &Peer) -> Result<i32> {
...
async fn main() -> Result<()> {
    ...
    let updates_rx = client
        .take_updates()
        .ok_or_else(|| Error::TelegramError("Updates stream already taken. Restart the probe.".into()))?;
            ...
            while Instant::now() < deadline {
                ...
                match tokio::time::timeout(wait_for, updates.next()).await {
                    Ok(Ok(Update::NewMessage(msg))) => {
                        if msg.peer_id() != target_peer_id || msg.outgoing() || msg.id() <= baseline_id {
                            continue;
                        }
    ...
    if args.json {
        ...
        for result in &results {
            ...
            if result.timed_out {
                println!("{} -> timed out after {}ms", result.command, result.duration_ms);
            } else if let Some(resp) = &result.response {
                println!("{} -> {} ({}ms)", result.command, resp, result.duration_ms);
            } else {
                println!("{} -> no response ({}ms)", result.command, result.duration_ms);
            }
        }
    }
