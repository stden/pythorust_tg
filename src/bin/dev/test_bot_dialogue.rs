//! Utility for testing a Telegram bot with dialogue analysis through OpenAI.
//!
//! Usage:
//!   cargo run --release --bin test_bot_dialogue -- --bot @example_bot --file dialogue.md
//!   cargo run --release --bin test_bot_dialogue -- --bot @example_bot --user-id USER_ID
//!   cargo run --release --bin test_bot_dialogue -- --bot @example_bot --interactive
...
/// Test a Telegram bot with AI dialogue analysis.
#[derive(Parser, Debug)]
#[command(name = "test_bot_dialogue")]
#[command(about = "Analyze Telegram bot dialogues through OpenAI")]
struct Args {
    /// Bot name (for example, @example_bot).
    #[arg(short, long)]
    bot: String,

    /// Path to a dialogue file (.md or .txt).
    #[arg(short, long)]
    file: Option<PathBuf>,

    /// User ID for loading a dialogue from MySQL.
    #[arg(short, long)]
    user_id: Option<i64>,
    ...
    /// Print only issues (without recommendations).
    #[arg(long)]
    problems_only: bool,
...
impl OpenAIAnalyzer {
    fn from_env(model: String) -> Result<Self, String> {
        let api_key = env::var("OPENAI_API_KEY").map_err(|_| "OPENAI_API_KEY не установлен")?;
    ...
    async fn analyze_dialogue(
        ...
        let bot_messages = lines
            .iter()
            .filter(|l| l.contains("Бот:") || l.contains("Bot:") || l.contains("Алина:"))
            .count();
...
fn print_analysis(analysis: &DialogueAnalysis, problems_only: bool) {
    ...
    println!(
        "   {} Телефон запрошен",
        if funnel.phone_requested { "✅" } else { "❌" }
    );
    println!(
        "   {} Телефон получен",
        if funnel.phone_collected { "✅" } else { "❌" }
    );
...
async fn load_dialogue_from_mysql(user_id: i64, bot_name: &str) -> Result<String, String> {
    ...
    if messages.is_empty() {
        return Err(format!(
            "Диалоги для user_id={} и бота {} не найдены",
            user_id, bot_name
        ));
    }
