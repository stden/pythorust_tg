...
use telegram_reader::grammers_client::types::Peer;
...
async fn main() -> Result<()> {
    ...
    while let Some(dialog) = dialogs.next().await? {
        ...
        while let Some(m) = messages.next().await? {
            ...
            let media_type = if m.photo().is_some() {
                Some("photo")
            ...
            let sender_id = m.sender().map(|s| match s {
                Peer::User(u) => u.raw.id() as i64,
                Peer::Channel(c) => c.raw.id,
                Peer::Group(g) => match &g.raw {
                    grammers_tl_types::enums::Chat::Chat(c) => c.id,
            ...
            match conn.exec_drop(
                ...
                (
                    ...
                    m.text(),
                    &date_str,
                    m.reply_to().map(|r| r.reply_to_msg_id as i64),
                    None::<i64>, // forward_from_id simplified
                    m.views().unwrap_or(0) as i32,
                    m.forwards().unwrap_or(0) as i32,
                    reactions_count,
                    &reactions_json,
                    media_type,
                ),
    ...
    println!("\n🎉 Done! Total loaded: {} messages from {} chats", total_inserted, processed_chats);
