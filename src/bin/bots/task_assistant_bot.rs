//! Task Assistant Telegram Bot for task automation.
//!
//! Features:
//! - N8N monitoring and management
//! - Backup creation and viewing
//! - AI consultations
//! - Server monitoring
//!
//! Usage:
...
async fn check_n8n_health() -> (String, Option<u16>, Option<String>) {
    ...
    let n8n_url = std::env::var("N8N_URL").unwrap_or_else(|_| "http://localhost:5678".to_string());
    match client
        .get(format!("{}/healthz", n8n_url.trim_end_matches('/')))
        .send()
        .await
    {
        Ok(resp) => {
...
fn restart_command() -> Vec<String> {
    env::var("N8N_RESTART_COMMAND")
        .unwrap_or_else(|_| "systemctl restart n8n".to_string())
        .split_whitespace()
        .map(str::to_string)
        .collect()
}
...
fn backup_dir() -> PathBuf {
    PathBuf::from(env::var("BACKUP_DIR").unwrap_or_else(|_| "/srv/backups/n8n".to_string()))
}

/// Restart N8N service.
async fn restart_n8n_service() -> (bool, Option<String>) {
    let parts = restart_command();
    let Some((program, args)) = parts.split_first() else {
        return (false, Some("N8N_RESTART_COMMAND is empty".to_string()));
    };
...
mod tests {
    ...
    #[test]
    fn restart_command_uses_env_value() {
        let _guard = EnvGuard::set("N8N_RESTART_COMMAND", "echo restart-n8n");

        assert_eq!(restart_command(), vec!["echo", "restart-n8n"]);
    }

    #[test]
    fn backup_dir_uses_env_value() {
        let _guard = EnvGuard::set("BACKUP_DIR", "/tmp/custom-n8n-backups");

        assert_eq!(backup_dir(), Path::new("/tmp/custom-n8n-backups"));
    }
}
