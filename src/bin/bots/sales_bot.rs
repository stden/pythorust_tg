//! Sales Bot (Rust)
//!
//! Telegram бот-продавец массажных кресел (GenericBrand) с логированием в MySQL
//! и A/B тестированием промптов. Переписан с Python-версии `sales_bot.py`.
...
use telegram_reader::mysql_async::{prelude::*, Pool};
use teloxide::dispatching::UpdateFilterExt;
use teloxide::prelude::*;
use teloxide::types::{Message, ReplyParameters, User};
use tracing::{error, info};

const BOT_NAME: &str = "sales_bot";

const SALES_SYSTEM_PROMPT: &str = r#"Ты - профессиональный консультант по массажным креслам компании GenericBrand.
...
ЛИНЕЙКА ПРОДУКТОВ:
- GenericBrand Premium R5: до 120 тыс, базовый 3D-массаж
- GenericBrand Premium R7: до 200 тыс, 4D-массаж, нулевая гравитация, Bluetooth
- GenericBrand Premium R9: до 300 тыс, топовый 4D, растяжка, все функции

Всегда старайся продать модель в рамках бюджета клиента."#;

const FAST_CLOSE_PROMPT: &str = r#"Ты — sales closer по массажным креслам GenericBrand.
...
const STORY_PROOF_PROMPT: &str = r#"Ты — консультант GenericBrand, работаешь по SPIN + social proof.
...
async fn main() -> Result<()> {
    ...
    let token = std::env::var("SALES_BOT_TOKEN")
        .context("SALES_BOT_TOKEN not set in environment (.env)")?;
    ...
    let experiment_name =
        std::env::var("SALES_PROMPT_EXPERIMENT").unwrap_or_else(|_| "sales_prompt_ab".to_string());
...
async fn handle_start(bot: &Bot, state: Arc<AppState>, msg: &Message) -> Result<()> {
    ...
    let recommendation = "Пока вы отвечаете, предварительно рекомендую линейку GenericBrand Premium: компактные модели с 4D-массажем и прогревом, подходят для ежедневного использования. Подберу точную модель, когда узнаю ваши вводные.";
