...
fn load_config() -> Config {
    ...
    match std::fs::read_to_string(path) {
        Ok(content) => match serde_yaml::from_str(&expand_env_vars(&content)) {
            Ok(cfg) => {
...
mod tests {
    ...
    #[test]
    fn expands_env_placeholders_in_yaml_values() {
        let _guard = EnvGuard::set("N8N_RESTART_COMMAND", "echo restart");

        let expanded = expand_env_vars("restart_command: ${N8N_RESTART_COMMAND}");

        assert_eq!(expanded, "restart_command: echo restart");
    }
}
