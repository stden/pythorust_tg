...
//!
//! Telegram bankruptcy consultant bot with MySQL logging.
//! Rewritten from the Python version `credit_expert_bot.py`.
...
const CREDIT_EXPERT_SYSTEM_PROMPT: &str = r#"Ты — кредитный эксперт.
...
ШАГ 1: ЗНАКОМСТВО
"Добрый день. Я кредитный эксперт. Как могу к вам обращаться?"
...
- "Подумаю": "Хорошо. Учтите, что пени и проценты продолжают расти. Отвечу, когда решите."
...
async fn handle_start(bot: Bot, msg: Message, state: AppState) -> Result<()> {
    let user = msg.from.as_ref().context("No user in message")?;
    ...
    // Send greeting
    let greeting = "Добрый день. Я кредитный эксперт. Как могу к вам обращаться?";
...
mod tests {
    #[test]
    fn source_text_does_not_contain_personal_markers() {
        let source = include_str!("credit_expert_bot.rs");
        let banned = [
            ["Дар", "ья"].concat(),
            ["D", "aria"].concat(),
            ["D", "asha"].concat(),
        ...
        for marker in banned {
            assert!(!source.contains(&marker), "found marker: {marker}");
        }
    }
}
