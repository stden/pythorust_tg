//! Community Game Bot (Rust)
//!
//! Bot for vibe-coding parties: lobby launch, answer collection, voting, and
//! user/message logging in MySQL.
...
const DEFAULT_BOT_NAME: &str = "CommunityGameBot";

fn bot_name() -> String {
    std::env::var("COMMUNITY_GAME_BOT_NAME").unwrap_or_else(|_| DEFAULT_BOT_NAME.to_string())
}
...
async fn main() -> Result<()> {
    ...
    let token = std::env::var("COMMUNITY_GAME_BOT_TOKEN")
        .map_err(|_| anyhow!("COMMUNITY_GAME_BOT_TOKEN not set in environment"))?;
...
async fn handle_message(bot: Bot, state: AppState, msg: Message) -> ResponseResult<()> {
    ...
    let user_id = msg
        ...
        .map(|u| u.id.0 as i64)
        .unwrap_or(chat_id.0);
    let user_name = display_name(msg.from.as_ref());

    state.db.save_user_if_needed(msg.from.as_ref()).await;
    state
        .db
        .log_message(
            msg.id.0 as i64,
    ...
    if is_command(text, "start") || is_command(text, "help") {
        let reply = "👋 Я Community Game Bot. Запускаю вайбкодинг-пати, собираю ответы и считаю голоса.\n\
                     \nЧто умею:\n\
...
fn extract_submission(text: &str) -> Option<&str> {
    if let Some(rest) = text.strip_prefix("/vibe") {
        ...
        let rest = rest.strip_prefix(&mention).unwrap_or(rest).trim_start();
        if rest.is_empty() {
            None
        } else {
            Some(rest)
...
fn parse_allowed_users() -> HashSet<i64> {
    std::env::var("COMMUNITY_GAME_ALLOWED_USERS")
        .unwrap_or_default()
        .split(',')
...
mod tests {
    ...
    #[test]
    fn bot_name_uses_env_value() {
        let _guard = EnvGuard::set("COMMUNITY_GAME_BOT_NAME", "EnvCommunityBot");

        assert_eq!(bot_name(), "EnvCommunityBot");
    }
}
