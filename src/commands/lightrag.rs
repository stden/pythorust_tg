...
//!
//! - Loads messages from MySQL (`telegram_messages`, `telegram_chats`)
//! - Builds documents with reactions and engagement
//! - Builds a lightweight RAG index with internal LightRAG
...
/// Type alias for the message row tuple to reduce complexity
...
fn row_to_message(row: Row) -> Option<RagMessage> {
    let (
        ...
        forwards,
        reply_to_msg_id,
    ): MessageRowTuple = from_row_opt(row).ok()?;
...
mod tests {
    ...
    fn naive_timestamp(secs: i64) -> NaiveDateTime {
    ...
    fn formats_document_with_engagement_and_reactions() {
        let msg = RagMessage {
            ...
            sender_name: Some("EntityA".into()),
            message_text: "Hello world!".into(),
            date: naive_timestamp(1_700_000_000),
        ...
        assert!(doc.contains("Отправитель: EntityA"));
    ...
    fn formats_source_with_fallback_sender() {
        let msg = RagMessage {
            ...
            date: naive_timestamp(1_700_000_100),
    ...
    fn sample_message(id: i64, sender: &str, text: &str) -> RagMessage {
        RagMessage {
            ...
            date: naive_timestamp(1_700_000_000 + id),
            reactions_count: None,
            reactions_raw: None,
            views: None,
            forwards: None,
    ...
    async fn build_retriever_indexes_messages_and_allows_retrieval() {
        let _guard = OpenAiKeyGuard::clear();

        let messages = vec![
            sample_message(
                1,
                "EntityA",
                "EntityA builds Rust bots for Telegram and open source projects.",
            ),
            sample_message(
                2,
                "EntityB",
                "EntityB enjoys gardening on weekends and writes Go code.",
            ),
        ];
        ...
        assert!(results.iter().any(|r| r.chunk.source.contains("msg_id=1")));
    }
