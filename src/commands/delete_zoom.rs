...
const ZOOM_URL_PATTERN: &str = "zoom.us";

fn contains_zoom_link(text: &str) -> bool {
    let pattern =
        std::env::var("ZOOM_URL_PATTERN").unwrap_or_else(|_| ZOOM_URL_PATTERN.to_string());
...
mod tests {
    ...
    #[test]
    fn contains_zoom_link_uses_env_pattern() {
        let _guard = EnvGuard::set("ZOOM_URL_PATTERN", "custom.zoom.example");

        assert!(contains_zoom_link(
            "Join: https://custom.zoom.example/j/123"
        ));
        assert!(!contains_zoom_link("Join: https://us06web.zoom.us/j/123"));
    }
