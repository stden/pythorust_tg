    ...
    let mut results: Vec<HuntResult> = user_data
        ...
        .filter(|u| {
            ...
            criteria.required_keywords.iter().all(|req| {
                u.keywords_found
                    .iter()
                    .any(|k| k.to_lowercase() == req.to_lowercase())
            })
        })
        .map(|u| {
...
pub fn print_results(results: &[HuntResult], limit: usize) {
    ...
    for (i, user) in results.iter().take(limit).enumerate() {
        ...
        println!(
            "   📊 Messages: {}, Keywords: {:?}",
            user.message_count, user.keywords_found
        );
        println!(
            "   🕐 Last active: {}",
            user.last_active.format("%d.%m.%Y %H:%M")
        );
...
pub fn export_csv(results: &[HuntResult]) -> String {
    let mut csv =
        String::from("user_id,username,full_name,message_count,score,last_active,keywords\n");
