...
//!
//! Usage: telegram_reader like --chat "chat_alpha" --user "example_user" --emoji "❤️"
...
mod tests {
    ...
    fn matcher_by_username() {
        let matcher = SenderMatcher::from_input("@sample_handle");
        let sender = sender_info(Some(1), Some("sample_handle"), "example_sender");
        assert!(matcher.matches(&sender));

        let other = sender_info(Some(1), Some("not_match"), "example_sender");
        assert!(!matcher.matches(&other));
    }

    #[test]
    fn matcher_by_name_substring() {
        let matcher = SenderMatcher::from_input("example");
        let sender = sender_info(Some(1), Some("personexample"), "example_sender");
        assert!(matcher.matches(&sender));

        let other = sender_info(Some(1), Some("sample_handle"), "sample_client");
        assert!(!matcher.matches(&other));
    }
    ...
    #[test]
    fn parse_flood_wait_seconds_parses_flood_wait_prefix() {
        assert_eq!(parse_flood_wait_seconds("FLOOD_WAIT_15"), Some(15));
        assert_eq!(
            parse_flood_wait_seconds("RPCError: FLOOD_WAIT_120 (cause)"),
            Some(120)
        );
    }

    #[test]
    fn parse_flood_wait_seconds_parses_value_prefix() {
        assert_eq!(
            parse_flood_wait_seconds("rpc error value: 42 exceeded"),
            Some(42)
        );
        assert_eq!(parse_flood_wait_seconds("value: 7 seconds"), Some(7));
    }

    #[test]
    fn parse_flood_wait_seconds_returns_none_on_invalid() {
        assert_eq!(parse_flood_wait_seconds("FLOOD_WAIT_"), None);
        assert_eq!(parse_flood_wait_seconds("value: none"), None);
        assert_eq!(parse_flood_wait_seconds("no wait info"), None);
    }

    #[test]
    fn reaction_matches_emoji_only_for_emoji_reactions() {
        let emoji = tl::enums::Reaction::Emoji(tl::types::ReactionEmoji {
            emoticon: "🔥".to_string(),
        });
        let custom =
            tl::enums::Reaction::CustomEmoji(tl::types::ReactionCustomEmoji { document_id: 123 });
        let paid = tl::enums::Reaction::Paid;

        assert!(reaction_matches_emoji(&emoji, "🔥"));
        assert!(!reaction_matches_emoji(&emoji, "❤️"));
        assert!(!reaction_matches_emoji(&custom, "🔥"));
        assert!(!reaction_matches_emoji(&paid, "🔥"));
    }

    #[test]
    fn sender_matcher_description_formats_variants() {
        let id = SenderMatcher::from_input("123");
        let username = SenderMatcher::from_input("@UserName");
        let name = SenderMatcher::from_input("example_sender");

        assert_eq!(id.description(), "ID 123");
        assert_eq!(username.description(), "@username");
        assert_eq!(name.description(), "example_sender");
    }
}
