...
impl LinearBot {
    ...
    fn merge_description(description: Option<&str>, reply_text: Option<&str>) -> Option<String> {
        ...
        match (description, reply_text) {
            (Some(desc), Some(reply)) => Some(format!(
    ...
    pub async fn handle_message(
        ...
        Ok(format!("Создана задача {} {}", identifier, url)
            .trim()
            .to_string())
    }
}
...
mod tests {
    ...
    fn build_bot(config: LinearBotConfig) -> LinearBot {
        let pattern = format!(r"^{}\s+(.+)", regex::escape(&config.command_prefix));
        let command_pattern = Regex::new(&pattern).expect("regex");
        let linear = LinearClient::new("test-key").expect("linear client");
    ...
    #[test]
    fn config_from_env_defaults_command_prefix_when_missing() {
        let _lock = ENV_LOCK.lock().unwrap();
        let _guard = EnvGuard::remove("LINEAR_COMMAND_PREFIX");

        let config = LinearBotConfig::from_env();
        assert_eq!(config.command_prefix, "!linear");
    }

    #[test]
    fn config_from_env_trims_command_prefix() {
        let _lock = ENV_LOCK.lock().unwrap();
        let _guard = EnvGuard::set("LINEAR_COMMAND_PREFIX", "  !do  ");

        let config = LinearBotConfig::from_env();
        assert_eq!(config.command_prefix, "!do");
    }

    #[test]
    fn config_from_env_parses_default_priority() {
        let _lock = ENV_LOCK.lock().unwrap();
        let _guard = EnvGuard::set("LINEAR_DEFAULT_PRIORITY", "3");
    ...
    #[test]
    fn config_from_env_ignores_invalid_default_priority() {
        let _lock = ENV_LOCK.lock().unwrap();
        let _guard = EnvGuard::set("LINEAR_DEFAULT_PRIORITY", "not-a-number");
    ...
    #[test]
    fn config_from_env_parses_allowed_senders() {
        let _lock = ENV_LOCK.lock().unwrap();
        let _guard = EnvGuard::set("LINEAR_ALLOWED_SENDERS", " 1, 2, x, , 3 ");

        let config = LinearBotConfig::from_env();
        assert!(config.allowed_sender_ids.contains(&1));
        assert!(config.allowed_sender_ids.contains(&2));
        assert!(config.allowed_sender_ids.contains(&3));
        assert_eq!(config.allowed_sender_ids.len(), 3);
    }

    #[test]
    fn matches_extracts_payload_after_prefix() {
        let bot = build_bot(LinearBotConfig {
            command_prefix: "!linear".to_string(),
            ..LinearBotConfig::default()
        });

        assert_eq!(bot.matches("!linear hello"), Some("hello".to_string()));
        assert_eq!(
            bot.matches("!linear   hi there"),
            Some("hi there".to_string())
        );
        assert_eq!(bot.matches("!linear"), None);
        assert_eq!(bot.matches("prefix !linear hi"), None);
    }

    #[test]
    fn is_sender_allowed_allows_everyone_when_list_empty() {
        let bot = build_bot(LinearBotConfig::default());
        assert!(bot.is_sender_allowed(1));
        assert!(bot.is_sender_allowed(999));
    }

    #[test]
    fn is_sender_allowed_filters_when_allowlist_set() {
        ...
        assert!(bot.is_sender_allowed(10));
        assert!(bot.is_sender_allowed(20));
        assert!(!bot.is_sender_allowed(30));
    }
}
