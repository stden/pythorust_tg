...
mod tests {
    ...
    #[test]
    fn openai_client_uses_base_url_from_env() {
        let _lock = ENV_LOCK.lock().unwrap();
        let _guard = EnvGuard::set("OPENAI_BASE_URL", "https://openai.example.test/v1");
    ...
    fn openai_client_stores_base_url() {
        let _lock = ENV_LOCK.lock().unwrap();
        let _guard = EnvGuard::set("OPENAI_BASE_URL", "https://api.openai.com/v1");
