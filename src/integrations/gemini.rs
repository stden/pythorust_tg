...
//!
//! Supports:
//! - Gemini 2.0/2.5/3.0 Flash and Pro
//! - Streaming responses
//! - Vision (images)
...
fn gemini_base_url() -> String {
    env::var("GEMINI_BASE_URL").unwrap_or_else(|_| GEMINI_API_URL.to_string())
}
...
mod tests {
    ...
    #[test]
    fn gemini_client_uses_base_url_from_env() {
        let _lock = ENV_LOCK.lock().unwrap();
        let _guard = EnvGuard::set("GEMINI_BASE_URL", "https://gemini.example.test/v1beta");

        let client = GeminiClient::new("test_key", "model").unwrap();
