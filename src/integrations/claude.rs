...
fn claude_base_url() -> String {
    env::var("ANTHROPIC_BASE_URL").unwrap_or_else(|_| CLAUDE_API_URL.to_string())
}
...
impl ClaudeClient {
    ...
    /// Simple chat: send a message and receive a response.
    ...
    /// Sales agent (uses a prompt from file).
...
mod tests {
    ...
    #[test]
    fn claude_client_uses_base_url_from_env() {
        let _lock = ENV_LOCK.lock().unwrap();
        let _guard = EnvGuard::set("ANTHROPIC_BASE_URL", "https://anthropic.example.test/v1");

        let client = ClaudeClient::new("test_key", "model").unwrap();

        assert_eq!(client.base_url, "https://anthropic.example.test/v1");
    }
    ...
    fn claude_client_stores_base_url() {
        let _lock = ENV_LOCK.lock().unwrap();
        let _guard = EnvGuard::set("ANTHROPIC_BASE_URL", "https://api.anthropic.com/v1");
