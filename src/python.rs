...
mod tests {
    ...
    fn test_contains_phone_valid() {
        assert!(contains_phone("+7 999 123-45-67"));
        assert!(contains_phone("Мой телефон: 89991234567"));
        assert!(contains_phone("Call me at (123) 456-7890"));
    }
    ...
    fn test_detect_conversion_phone() {
        assert_eq!(
            detect_conversion("+7 999 123-45-67"),
    ...
    fn test_detect_conversion_checkout() {
        // "давай" + delivery keyword without purchase intent keywords
        // Using "адрес" as delivery keyword and "давай" as action
        assert_eq!(
    ...
    fn test_attach_message_detects_phone() {
        ...
        metrics.attach_message("Мой телефон +7 999 123-45-67", "incoming");
    ...
    fn test_sanitize_filename_boundary_distinguishes_gt_from_gte() {
        ...
        let name = "1234567890"; // exactly 10 chars
        let result = sanitize_filename(Some(name), "fallback", Some(10));

        // The key assertion: with >= mutation, this would still return "1234567890"
    ...
    fn test_detect_conversion_delivery_without_action() {
        // "адрес" is a delivery keyword, but no action word (давай/офор)
        assert_eq!(detect_conversion("мой адрес для справки"), None);
    }

    #[test]
    fn test_detect_conversion_action_without_delivery() {
        // "давай" is an action word, but no delivery keyword
