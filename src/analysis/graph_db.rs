...
impl GraphStore {
    ...
    pub async fn find_interacting_users(
        &self,
        user_id: i64,
        limit: usize,
    ) -> Result<Vec<UserNode>> {
...
mod tests {
    ...
    #[test]
    fn test_graph_stats_custom() {
        let stats = GraphStats {
            user_count: 100,
            chat_count: 50,
            message_count: 5000,
            relation_count: 10000,
        };
        assert_eq!(stats.user_count, 100);
        assert_eq!(stats.chat_count, 50);
        assert_eq!(stats.message_count, 5000);
        assert_eq!(stats.relation_count, 10000);
    }
    ...
    async fn test_graph_store_from_env_requires_password() {
        std::env::remove_var("NEO4J_PASSWORD");
        let result = GraphStore::from_env().await;
        assert!(result.is_err());
    }

    #[test]
    fn test_graph_stats_zero_values() {
        let stats = GraphStats::default();
        assert_eq!(stats.user_count, 0);
        assert_eq!(stats.chat_count, 0);
        assert_eq!(stats.message_count, 0);
    ...
    fn test_graph_stats_all_fields_accessible() {
        ...
        let total =
            stats.user_count + stats.chat_count + stats.message_count + stats.relation_count;
        assert_eq!(total, 110);
    }
