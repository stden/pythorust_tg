...
mod tests {
    ...
    fn make_service(model: &str) -> EmbeddingService {
        let client = OpenAIClient::with_config(OpenAIConfig::new().with_api_key("test_key"));
        EmbeddingService {
            client,
    ...
    fn dimension_returns_expected_values() {
        let default = make_service("text-embedding-3-small");
        assert_eq!(default.dimension(), 1536);

        let large = make_service("text-embedding-3-large");
        assert_eq!(large.dimension(), 3072);

        let ada = make_service("text-embedding-ada-002");
        assert_eq!(ada.dimension(), 1536);

        let custom = make_service("custom-model");
