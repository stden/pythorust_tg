...
## Current State
- Basic auto-responder exists in Rust (`src/bin/chat/autoanswer.rs`) without sales scripts and catalog, without CRM/Sheets lead logging.
- Telegram reading is Rust-first (`telegram_reader read`, `telegram_reader tg`) without sales triggers and CTAs.
...
## References
- `src/bin/chat/autoanswer.rs`, `src/commands/read.rs`, `src/bin/chat/tg.rs` — current Rust Telegram logic.
