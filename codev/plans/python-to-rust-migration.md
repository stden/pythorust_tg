...
- Python legacy: `read_example_chat.py`, `export_user_simple.py`, `read_specific_chat.py`, assorted helpers.
...
### Bots
- Rust: `src/bin/sales_bot.rs` (MySQL logging + A/B prompts).
...
- Actions: migrate Credit Expert bot to Rust (teloxide), share logging/AB infra with SALES bot, deprecate legacy base once Rust bots cover flows.
...
- Migrate Credit Expert bot to Rust with shared MySQL schema; reuse A/B infra from SALES bot.
