...
- **Files:** `src/session.rs`, `src/bin/dev/init_session.rs` (legacy `telegram_session.py` removed)
- **Solution:** Separate session files: Python uses `telegram_session_py.session`, Rust uses `telegram_session.session`
- **Acceptance:** ✅ Rust session initialization and locking are documented; legacy Python session work is superseded
...
- **Files:** `src/bin/moderation/delete_unanswered.rs`
...
- **Files:** `src/bin/bots/credit_expert_bot.rs`, service file TBD
- **Dependencies:** MySQL schema from `migrations/002_create_bot_tables.sql`
...
- **Files:** `src/bin/bots/sales_bot.rs` (service file TBD)
