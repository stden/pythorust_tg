...
class TestCheckEKS:
    ...
    def sample_clusters(self):
        ...
        return {"clusters": ["prod-cluster", "staging-cluster", "dev-cluster"]}
    ...
    def sample_nodegroups(self):
        ...
        return {"nodegroups": ["workers-1", "workers-2", "spot-workers"]}
    ...
    def test_check_nodegroup_health(self, mock_boto3_client, sample_nodegroup_details):
        ...
        nodegroup = mock_boto3_client.describe_nodegroup(clusterName="prod-cluster", nodegroupName="workers-1")[
            "nodegroup"
        ]
    ...
    def test_check_cluster_addons(self, mock_boto3_client):
        ...
        addon_response = {
            "addons": [
                {"addonName": "vpc-cni", "addonVersion": "v1.12.6-eksbuild.1", "status": "ACTIVE"},
                {"addonName": "kube-proxy", "addonVersion": "v1.27.1-eksbuild.1", "status": "ACTIVE"},
                {"addonName": "coredns", "addonVersion": "v1.10.1-eksbuild.1", "status": "ACTIVE"},
            ]
        }

        mock_boto3_client.list_addons.return_value = {"addons": ["vpc-cni", "kube-proxy", "coredns"]}
        mock_boto3_client.describe_addon.side_effect = [{"addon": addon} for addon in addon_response["addons"]]
        ...
        for addon_name in addon_names:
            addon = mock_boto3_client.describe_addon(clusterName="prod-cluster", addonName=addon_name)["addon"]
            addon_statuses.append(
                {"name": addon["addonName"], "version": addon["addonVersion"], "is_active": addon["status"] == "ACTIVE"}
            )
    ...
    def test_check_multiple_clusters(self, mock_boto3_client, sample_clusters):
        ...
        cluster_statuses = {"prod-cluster": "ACTIVE", "staging-cluster": "ACTIVE", "dev-cluster": "UPDATING"}

        def describe_cluster_side_effect(name):
            return {"cluster": {"name": name, "status": cluster_statuses[name], "version": "1.27"}}
        ...
        for cluster_name in clusters:
            ...
            results.append(
                {"name": cluster["name"], "status": cluster["status"], "is_ready": cluster["status"] == "ACTIVE"}
            )
    ...
    def test_error_handling_cluster_not_found(self, mock_boto3_client):
        ...
        mock_boto3_client.describe_cluster.side_effect = ClientError(
            {"Error": {"Code": "ResourceNotFoundException", "Message": "Cluster not found"}}, "DescribeCluster"
        )
    ...
    def test_check_cluster_authentication(self, mock_boto3_client):
        ...
        identity_config = {"oidc": {"issuer": "https://oidc.eks.us-east-1.amazonaws.com/id/EXAMPLE"}}

        cluster_with_oidc = {"cluster": {"name": "prod-cluster", "identity": identity_config, "status": "ACTIVE"}}
    ...
    def test_format_output(self):
        ...
        results = {
            "clusters": [
                {"name": "prod-cluster", "status": "ACTIVE", "version": "1.27", "nodegroups": 3, "issues": []},
                {
                    "name": "dev-cluster",
                    "status": "UPDATING",
                    "version": "1.26",
            ...
            "summary": {"total_clusters": 2, "active_clusters": 1, "clusters_with_issues": 1},
        }
