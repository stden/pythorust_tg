...
@pytest.fixture
def config(tmp_path):
    return AnalyzerConfig(output_dir=tmp_path / "output", verbose=False, llm_provider="openai", model="gpt-3.5-turbo")
...
async def test_analyze_chat_success(analyzer):
    ...
    with (
        patch("chat_analysis.analyzer.MessageFetcher", return_value=mock_fetcher),
