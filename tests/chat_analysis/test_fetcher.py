...
def config():
    return AnalyzerConfig(
        message_limit=10, days_back=7, min_message_length=5, include_media=False, exclude_bots=True, verbose=False
    )
...
def fetcher(config):
    with patch.dict(
        "os.environ", {"TELEGRAM_API_ID": "12345", "TELEGRAM_API_HASH": "hash", "TELEGRAM_SESSION_FILE": "test_session"}
    ):
...
async def test_get_sender_name(fetcher):
    ...
    msg = MagicMock(spec=Message)
    msg.sender = user
    assert await fetcher._get_sender_name(msg) == "Sample Doe"

    # User with username
    user.first_name = None
    user.last_name = None
    user.username = "sample_sender"
    assert await fetcher._get_sender_name(msg) == "@sample_sender"
...
async def test_format_message(fetcher):
    ...
    msg.reactions.results = [MagicMock(spec=ReactionCount, count=5), MagicMock(spec=ReactionCount, count=3)]
...
def test_format_messages_for_llm(fetcher):
    ...
    assert "[01.01.2023 12:00] EntityA: Hello [2 reactions]" in lines[0]
    assert "[01.01.2023 12:05] EntityB: World [media]" in lines[1]
...
async def test_get_messages(fetcher):
    ...
    with (
        patch.object(fetcher, "_should_include", side_effect=[True, False]),
        patch.object(
            fetcher,
            "_format_message",
            return_value=FormattedMessage(date=datetime.now(), sender_name="S", text="T", message_id=1),
        ),
    ):
