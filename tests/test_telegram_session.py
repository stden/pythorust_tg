...
class TestTelegramSession:
    ...
    async def test_connect_not_authorized(self, session, mock_telethon_client):
        ...
        mock_telethon_client.start.assert_called_once_with(
            phone="+1234567890"
        )
    ...
    async def test_get_messages(self, session, mock_telethon_client):
        ...
        # Mock messages
        messages = [
            MagicMock(id=1, text="Message 1"),
            MagicMock(id=2, text="Message 2")
        ]
        ...
        mock_telethon_client.get_messages.assert_called_once_with(
            "test_chat",
            limit=10
        )
    ...
    async def test_download_media_to_path(self, session, mock_telethon_client, tmp_path):
        ...
        result = await session.download_media(
            mock_message,
    ...
    async def test_delete_messages(self, session, mock_telethon_client):
        ...
        mock_telethon_client.delete_messages.assert_called_once_with(
            "test_chat",
            [1, 2, 3]
        )
    ...
    async def test_get_dialogs(self, session, mock_telethon_client):
        ...
        dialogs = [
            MagicMock(name="Chat 1", unread_count=5),
            MagicMock(name="Chat 2", unread_count=0)
        ]
...
class TestModuleFunctions:
    ...
    async def test_get_messages_function(self, mock_session_class):
        ...
        assert len(result) == 2
        mock_session.get_messages.assert_called_once_with(
            "test_chat",
            limit=10
        )
    ...
    async def test_delete_messages_function(self, mock_session_class):
        ...
        mock_session.delete_messages.assert_called_once_with(
            "test_chat",
            [1, 2, 3]
        )
