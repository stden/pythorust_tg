...
def _run_in_loop(coro):
    """Запускает coroutine, создавая event loop при необходимости."""
    try:
...
def step_user_exists_in_table(context, table):
    ...
    assert context.current_user.id in context.saved_users, f"Пользователь {context.current_user.id} не найден в {table}"
...
def step_user_name_equals(context, name):
    ...
    assert user_data["first_name"] == name, f"Имя {user_data['first_name']}, ожидалось {name}"
...
def step_username_updated(context, new_username):
    ...
    user_data = context.saved_users.get(context.current_user.id)
    assert user_data["username"] == new_username, f"Username {user_data['username']}, ожидалось {new_username}"
...
def step_message_saved_with_direction(context, direction):
    ...
    assert last_message["direction"] == direction, f"Направление {last_message['direction']}, ожидалось {direction}"
...
def step_message_text_equals(context, text):
    ...
    last_message = context.saved_messages[-1]
    assert last_message["text"] == text, f"Текст '{last_message['text']}', ожидалось '{text}'"
...
def step_session_created_with_state(context, state):
    ...
    assert session["state"] == state, f"Статус {session['state']}, ожидалось {state}"
...
def step_returns_messages(context, count):
    ...
    assert len(context.history) == count, f"Возвращено {len(context.history)} сообщений, ожидалось {count}"
...
def step_bot_sends_greeting(context, name):
    ...
    assert context.greeting_name == name, f"Имя в приветствии {context.greeting_name}, ожидалось {name}"
...
def step_response_reasonable_length(context):
    ...
    assert len(context.ai_response) < 2000, f"Ответ слишком длинный: {len(context.ai_response)} символов"


@then('первое сообщение имеет роль "{role}"')
def step_first_message_role(context, role):
    """Проверяем роль первого сообщения."""
    assert context.ai_messages[0]["role"] == role, f"Роль {context.ai_messages[0]['role']}, ожидалось {role}"
...
def step_system_prompt_contains(context, text):
    ...
    assert text in system_content, f"'{text}' не найден в системном промпте"
...
def step_roles_alternate(context, role1, role2):
    ...
    for i, msg in enumerate(context.ai_messages[1:]):
        ...
        assert msg["role"] in [role1, role2], f"Неизвестная роль: {msg['role']}"


@then("бот возвращает сообщение об ошибке")
def step_bot_returns_error(context):
    """Проверяем сообщение об ошибке."""
    assert context.ai_error is not None or "ошибка" in context.ai_response.lower(), "Сообщение об ошибке не возвращено"


@then('сообщение содержит "{text}"')
def step_message_contains(context, text):
    """Проверяем содержимое сообщения."""
    assert text in context.ai_response, f"'{text}' не найден в ответе"
...
def step_bot_recommends_model(context, model):
    ...
    assert context.recommended_model == model, f"Рекомендована модель {context.recommended_model}, ожидалось {model}"


@then("указывает цену до {price:d} тыс")
def step_price_indicated(context, price):
    """Проверяем указание цены."""
    assert context.price_range == price * 1000, f"Цена {context.price_range}, ожидалось {price * 1000}"
...
def step_bot_follows_strategy(context, stage):
    ...
    assert context.current_strategy is not None, f"Стратегия для этапа '{stage}' не определена"
