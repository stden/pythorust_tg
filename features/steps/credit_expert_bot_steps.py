...
def step_history_contains(context):
    for row in context.table:
        ...
        context.db_mock.save_message(
            context.current_user.id, len(context.saved_messages) + 1, row["content"], direction
        )
...
def step_user_sends_start(context):
    ...
    greeting = "Здравствуйте! Я кредитный эксперт. Вижу, что обратились по вопросу долгов. Помогу разобраться. Как к вам обращаться?"
...
def step_user_sends_text(context, text):
    ...
    # Call AI
    response = _run_in_loop(context.ai_mock.chat_completion(messages))
