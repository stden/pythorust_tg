# -*- coding: utf-8 -*-
"""Behave steps for testing the Sales Bot."""
...
def step_mysql_configured(context):
    ...
    def save_message(user_id, message_id, text, direction, bot_name="sales_bot", reply_to=None):
    ...
    def create_session(user_id, bot_name="sales_bot"):
    ...
    def get_session(user_id, bot_name="sales_bot"):
        for session in context.sessions.values():
            if session["user_id"] == user_id and session["is_active"]:
                return session
        return None

    def get_conversation_history(user_id, bot_name="sales_bot", limit=20):
...
def step_openai_configured(context):
    ...
    async def chat_completion(messages):
        ...
        response.choices[0].message.content = "Great choice! I recommend GenericBrand Premium R7."
        return response
    ...
    async def get_response():
        try:
            response = await context.ai_mock.chat_completion(
                [
                    {"role": "system", "content": "You are a GenericBrand consultant"},
...
def step_format_ai_messages(context):
    ...
    SALES_SYSTEM_PROMPT = "You are a professional massage chair consultant for GenericBrand."
...
@then("the bot recommends the GenericBrand Premium lineup")
def step_bot_recommends_relaxio(context):
    """Assert the bot recommends GenericBrand."""
...
def step_response_contains_products(context):
    ...
    assert "GenericBrand" in context.ai_response, "Response does not mention GenericBrand"
...
@then("the bot recommends GenericBrand Premium {model}")
...
# ========== Purchase scenarios: Given Steps ==========
...
@then("объясняет отличия GenericBrand")
