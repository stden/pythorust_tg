Feature: Credit Expert Bot Logic
  ...
  Scenario: Initial greeting
    ...
    And the bot should reply with "Здравствуйте! Я кредитный эксперт. Вижу, что обратились по вопросу долгов. Помогу разобраться. Как к вам обращаться?"

  Scenario: Qualification flow - User provides name
    Given a user "User123" exists
    And the user has started a conversation
    When the user sends "Меня зовут клиент"
    Then the bot should reply with text containing "Клиент, подскажите, вы уже решили заниматься вопросом с долгами или пока изучаете варианты?"

  Scenario: Qualification flow - User is studying options
    Given a user "User123" exists
    And the conversation history contains:
      | role      | content |
      | assistant | Клиент, подскажите, вы уже решили заниматься вопросом с долгами или пока изучаете варианты? |
