...
Feature: Sales Bot — AI consultant for massage chairs
  As a sales manager
  ...
  Scenario: Saving a new user
    Given a user with id 123456 and name "sample_client"
    When the bot saves the user to the database
    Then the user exists in table "bot_users"
    And the user name is "sample_client"

  Scenario: Updating an existing user
    Given a user with id 123456 already exists in the database
    And the user changed username to "sample_handle"
    When the bot saves the user to the database
    Then the user username is updated to "sample_handle"
  ...
  Scenario: /start sends a greeting
    Given a new user "sample_client" sent /start
    When the bot handles the command
    Then the bot sends a greeting with name "sample_client"
    And the bot asks clarifying questions
    And the bot recommends the GenericBrand Premium lineup
  ...
  Scenario: AI uses the system prompt
    ...
    And the system prompt contains "GenericBrand"
  ...
  Scenario: R9 model information
    ...
    Then the bot recommends GenericBrand Premium R9
  ...
  Scenario: Buying for yourself — office worker with back pain
    Given the customer "sample_client" works in an office 8+ hours a day
  ...
  Scenario: Buying as a gift for parents
    Given the customer "sample_client" is looking for a gift for parents
  ...
  Scenario: Negative review of a previous experience
    ...
    And it explains GenericBrand differences
