...
fn extractor_benchmark(c: &mut Criterion) {
    ...
    let base_text = "EntityA builds Rust bots for Telegram and graph pipelines \
        with EntityB and EntityC in Berlin while testing entity extraction speed."
        .repeat(32);
...
fn pyo3_text_benchmarks(c: &mut Criterion) {
    ...
    let phone_text = "+0 000 000-00-00 call me please";
...
fn pyo3_batch_benchmark(c: &mut Criterion) {
    ...
    let messages: Vec<(&str, bool)> = (0..1000)
        .map(|i| {
            let text = if i % 3 == 0 {
                "Message with phone +0 000 000-00-00"
            } else if i % 5 == 0 {
                "/command"
            } else {
