...
~50 binaries for specialized tasks: bots (`sales_bot`, `credit_expert_bot`, `community_game_bot`, `devops_ai_bot`), utilities (`delete_unanswered`, `bulk_reactions`, `http_bench`), data tools (`index_messages`, `export_chats_mysql`, `k8s_dash`), and more. These import from the `telegram_reader` library crate.
...
Remaining scripts to migrate: `chat_analysis`, `mcp_telegram_server.py`. Install deps: `uv sync`. Already migrated to Rust (`src/bin/`): `n8n_monitor`, `n8n_backup`, `sales_bot`, `credit_expert_bot`, `task_assistant_bot`, `devops_ai_bot`, `community_game_bot`, `ai_project_consultant`.
