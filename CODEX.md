...
| Sales Bot (Rust) | `src/bin/sales_bot.rs` | Rust bot with MySQL logging and A/B prompt variants |
...
### Prompt variants
Prompts are defined in `src/bin/sales_bot.rs` (`SALES_SYSTEM_PROMPT`, `FAST_CLOSE_PROMPT`, `STORY_PROOF_PROMPT`). A/B variants are assembled in `prompt_variants()`; adjust weights/temperature or text there. Experiments use `SALES_PROMPT_EXPERIMENT` (default `sales_prompt_ab`) and are stored in MySQL `bot_experiments`.
