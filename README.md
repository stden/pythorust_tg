...
Rust-first toolkit for Telegram automation. High-speed chat exports, AI auto-responder, reactions/digests/moderation/CRM parsing, LightRAG indexing, and ops helpers (N8N monitor/backup, HTTP bench, site monitor, k8s dashboard). Python remains only for legacy analysis/MCP/helpers while migration continues.
...
Build-health note: `Cargo.lock` is committed. The yanked `core2` transitive
dependency is handled by the temporary `vendor/core2-0.4.0` patch documented in
[AGENTS.md](AGENTS.md#6-build-health--known-issues).

### Python (legacy analysis/MCP/helpers)
```bash
...
src/bin/                # 52 specialized binaries grouped by category
prompts/                # System prompts used by analyzers/bots
chat_analysis/          # Legacy Python analyzer
migrations/             # SQL schema migrations (viral questions + bot tables)
scripts/ops/            # systemd/cron helper templates
...
|---------|----------|-----------------|--------|
| Chat export to Markdown | ✅ | Removed legacy scripts | Stable |
| AI auto-responder | ✅ | — | In progress (Rust improvements planned) |
| Linear integration | ✅ | Removed legacy helper | Stable |
| Reactions/likes | ✅ | — | Stable |
| Delete Zoom / cleanup | ✅ | Removed legacy scripts | Stable |
| AI digest / analysis | ✅ | Legacy analyzer | Stable |
| CRM parsing / user hunt | ✅ | — | New |
| N8N monitor/backup | ✅ CLI | Shell/systemd templates only | Stable |
